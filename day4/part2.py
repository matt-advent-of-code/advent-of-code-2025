...
def count_and_remove(rolls: [[str]]) -> int:
    accessible = 0
    to_remove : [(int, int)] = []
    for x in range(0, len(rolls)):
        for y in range(0, len(rolls[x])):
            is_paper_accessible = is_accessible(rolls, x, y)
            accessible += is_paper_accessible
            if is_paper_accessible:
